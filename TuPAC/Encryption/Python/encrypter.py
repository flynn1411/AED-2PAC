...
decrypted = "".join(list(map(lambda x: x ^ chr(ord(password)),bytearray(encripted))))
...
class Encryptor:
    ...
    def encrypt(self,phrase,password):
        return "".join(list(map(lambda x: chr(x ^ ord(password[0])),)))

    def decrypt(self,encriptedtext,password):
        return self.encrypt(encriptedtext,password)
        
    #def encryptM(self,phrase,password):
    #   c = 0
    #    for i in bytearray(phrase):
